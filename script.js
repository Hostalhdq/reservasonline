try {
    ...
    if (resultado.success) {
        mostrarMensaje(`
            ...
            ID: ${datos.id_reserva}<br>
            Total: $${total.toLocaleString()}<br>
            <small>Los datos han sido guardados en Google Sheets</small>
        `, 'success');
        ...
        mostrarMensaje("❌ Error al guardar en Google Sheets: " + (resultado.error || "Desconocido"), "error");
    }
} catch (error) {
    mostrarMensaje("❌ Error al conectar con Google Sheets: " + error.message, "error");
}
finally {
    // Restaurar botón
